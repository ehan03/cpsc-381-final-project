...
class BacktestFramework:
    ...
    def __init__(self, data, bankroll=100):
        ...
        fight_information = read_csv[['EVENT_ID','RED_FIGHTER_ODDS','BLUE_FIGHTER_ODDS','RED_WIN']].to_numpy()
        ...
        if fight_information.shape[0] == self.data.shape[0]:
            ...
            raise ValueError("The number of rows in the CSV does not match the number of rows in `data`.")
        self.bankroll = bankroll

    def split_by_event_id(self) -> list[np.array]:
    ...
    def test_events(self):
        ...
        for group in grouped_data:
            ...
            kelly = SimultaneousKelly(group[:,4], group[:,5], group[:,1], group[:,2], temporary_bank)
            red_wagers, blue_wagers = kelly()
            for red_wager, blue_wager, outcome, red_betting_odds, blue_betting_odds in zip(red_wagers, 
                                                                                           blue_wagers, 
                                                                                           group[:,3],
                                                                                           group[:,1],
                                                                                           group[:,2]):
