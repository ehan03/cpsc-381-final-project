        ...
        odds_features = pd.read_sql(
            ...
            stacked_odds_features AS (
              SELECT 
                ...
                AVG(WIN_FAVORITE) OVER(
                  PARTITION BY FIGHTER_ID 
                  ORDER BY 
                    rn ROWS BETWEEN UNBOUNDED PRECEDING 
                    AND 1 PRECEDING
                ) AS WIN_FAVORITE_RATE, 
                AVG(WIN_UNDERDOG) OVER(
                  PARTITION BY FIGHTER_ID 
                  ORDER BY 
                    rn ROWS BETWEEN UNBOUNDED PRECEDING 
                    AND 1 PRECEDING
                ) AS WIN_UNDERDOG_RATE, 
                AVG(LOSE_FAVORITE) OVER(
                  PARTITION BY FIGHTER_ID 
                  ORDER BY 
                    rn ROWS BETWEEN UNBOUNDED PRECEDING 
                    AND 1 PRECEDING
                ) AS LOSE_FAVORITE_RATE, 
                AVG(LOSE_UNDERDOG) OVER(
                  PARTITION BY FIGHTER_ID 
                  ORDER BY 
                    rn ROWS BETWEEN UNBOUNDED PRECEDING 
                    AND 1 PRECEDING
                ) AS LOSE_UNDERDOG_RATE, 
                AVG(ROI) OVER(
                  PARTITION BY FIGHTER_ID 
                  ORDER BY 
                    rn ROWS BETWEEN UNBOUNDED PRECEDING 
            ...
            SELECT 
              ...
              t1.RED_FIGHTER_ID, 
              t1.BLUE_FIGHTER_ID, 
              t2.FIGHTER_ODDS_AVERAGE - t3.FIGHTER_ODDS_AVERAGE AS FIGHTODDSIO_ODDS_AVERAGE_DIFF, 
              t2.OPPONENT_ODDS_AVERAGE - t3.OPPONENT_ODDS_AVERAGE AS FIGHTODDSIO_OPPONENT_ODDS_AVERAGE_DIFF, 
              t2.FIGHTER_IMPLIED_PROB_AVERAGE - t3.FIGHTER_IMPLIED_PROB_AVERAGE AS FIGHTODDSIO_IMPLIED_PROB_AVERAGE_DIFF, 
              t2.WIN_FAVORITE_RATE - t3.WIN_FAVORITE_RATE AS FIGHTODDSIO_WIN_FAVORITE_RATE_DIFF, 
              t2.WIN_UNDERDOG_RATE - t3.WIN_UNDERDOG_RATE AS FIGHTODDSIO_WIN_UNDERDOG_RATE_DIFF, 
              t2.LOSE_FAVORITE_RATE - t3.LOSE_FAVORITE_RATE AS FIGHTODDSIO_LOSE_FAVORITE_RATE_DIFF, 
              t2.LOSE_UNDERDOG_RATE - t3.LOSE_UNDERDOG_RATE AS FIGHTODDSIO_LOSE_UNDERDOG_RATE_DIFF, 
              t2.ROI_AVERAGE - t3.ROI_AVERAGE AS FIGHTODDSIO_ROI_AVERAGE_DIFF, 
