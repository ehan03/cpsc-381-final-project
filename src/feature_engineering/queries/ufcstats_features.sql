...
SELECT
  ...
  (CAST(t2.FIGHTER_CONTROL_TIME_SECONDS AS FLOAT) / (t2.FIGHTER_KNOCKDOWNS + t2.FIGHTER_TAKEDOWNS_LANDED + t2.FIGHTER_REVERSALS + 1)) - (CAST(t3.FIGHTER_CONTROL_TIME_SECONDS AS FLOAT) / (t3.FIGHTER_KNOCKDOWNS + t3.FIGHTER_TAKEDOWNS_LANDED + t3.FIGHTER_REVERSALS + 1)) AS UFCSTATS_CONTROL_TIME_SECONDS_PER_GROUND_EXCHANGE,
  CASE t1.RED_OUTCOME WHEN 'W' THEN 1 WHEN 'L' THEN 0 ELSE NULL END AS RED_WIN 
FROM
  main.UFCSTATS_BOUTS_OVERALL AS t1
